...
class DbManager(context: Context) :
    ...
    fun getAllTags(): List<Tag> {
        ...
        readableDatabase.rawQuery("SELECT * FROM ${Tag.TABLE_NAME}", arrayOf()).use { cursor ->
            if (cursor != null)
                while (cursor.moveToNext())
                    tags.add(Tag(this, cursor))
        }
        return tags
    }

    fun entryById(entryId: Int): Entry? {
        readableDatabase.rawQuery(
            "SELECT * FROM ${Entry.TABLE_NAME} WHERE ${Entry.COLUMN_NAME_ID} = ?",
            arrayOf(entryId.toString())
        ).use { cursor ->
            return if (cursor.moveToFirst()) Entry(this, cursor) else null
        }
    }

    fun tagByName(name: String): Tag? {
        readableDatabase.rawQuery(
            "SELECT * FROM ${Tag.TABLE_NAME} WHERE ${Entry.COLUMN_NAME_NAME} = ?",
            arrayOf(name)
        ).use { cursor ->
            return if (cursor.moveToFirst()) Tag(this, cursor) else null
        }
    }
